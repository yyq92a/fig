...
range_color=(0,8000),#颜色条变化范围，单位百万吨
color_continuous_scale=px.colors.sequential.Reds,#选择颜色条色系
color_continuous_midpoint=np.average(df_co2['co2'], weights=df_co2['population']),#调整颜色策略
)

fig.update_layout(coloraxis_colorbar=dict(title='CO2(Million Tons)')) #增加一个单位到颜色条
